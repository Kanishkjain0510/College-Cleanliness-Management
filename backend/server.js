...
const db = mysql.createPool({
  ...
  user: 'root',
  password: '123456789',
  database: 'JKLU_Cleanliness_Management',
...
app.post("/api/cleanliness-issues", async (req, res) => {
  const { area_id, description, reported_time, status, priority } = req.body;

  try {
    const [result] = await db.query(
      'INSERT INTO CLEANLINESS_ISSUE (area_id, description, reported_time, status, priority) VALUES (?, ?, ?, ?, ?)',
      [area_id, description, reported_time, status, priority]
    );
